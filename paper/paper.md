...
`VE` was originally developed to support the simulation and optimization of the low-temperature conversion of biomass to ethanol through aerobic fermentation. This bioconversion process was modeled by linking three separately developed computational unit operations of bioconversion (Figure \ref{fig:VE_diagram}), namely:  (i) the _pretreatment_ of the feedstock to make cellulose more accessible [@sitaraman_multiphysics_2015], (ii) an _enzymatic hydrolysis_ step to digest cellulose into sugars [@sitaraman_coupled_2019], and (iii) a _bioconversion_ step to convert sugars into products in a reactor [@rahimi_computational_2018].  Finally, the capital and operating costs of the process, and the ethanol product’s subsequent minimum feasible selling price through discounted cash flow analysis, were calculated using an Aspen Plus process simulation with techno-economic analysis (TEA) [@humbird2011process]. 
